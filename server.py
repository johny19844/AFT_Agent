...
def upload_model():
    ...
    try:
        ...
        if file and file.filename.endswith('.gguf'):
            ...
            add_agent_log(f"📁 Файл модели загружен: {filename}", "success")
            add_agent_log(f"📂 Полный путь: {absolute_path}", "info")
        ...
        add_agent_log(f"❌ Ошибка загрузки файла: {str(e)}", "error")
...
def start_agent():
    ...
    # Проверяем существование agent_v023.py
    if not os.path.exists('agent_v023.py'):
        add_agent_log("❌ Файл agent_v023.py не найден!", "error")
        return jsonify({
            "status": "error", 
            "message": "Файл agent_v023.py не найден! Убедитесь, что он в той же папке."
        })
    
    # Получаем настройки из интерфейса
    config = request.json
    add_agent_log(f"🔄 Получены настройки от интерфейса", "info")
    add_agent_log(f"   Модель: {config['model_path']}", "info")
    add_agent_log(f"   Репозиторий сценариев: {config['scenario_repo']}", "info")
    add_agent_log(f"   Репозиторий AFT: {config['aft_repo']}", "info")
    add_agent_log(f"   Интервал сканирования: {config['scan_interval']} сек", "info")
    
    # Очищаем предыдущие логи
    agent_logs = []
    last_log_count = 0
    
    add_agent_log("🚀 Запуск агента с новыми настройками...", "info")
    ...
    if not absolute_model_path:
        add_agent_log(f"❌ Файл модели не найден: {model_filename}", "error")
        add_agent_log("🔍 Искали в: ./models/, ./, ../", "info")
        return jsonify({
            "status": "error", 
            "message": f"Файл модели не найден: {model_filename}. Проверьте путь или загрузите файл через интерфейс."
        })
    
    add_agent_log(f"✅ Найден файл модели: {absolute_model_path}", "success")
    ...
    try:
        ...
        add_agent_log(f"🔧 Команда запуска: {' '.join(cmd)}", "info")
        ...
        add_agent_log("✅ Агент успешно запущен!", "success")
        add_agent_log("📖 Начинаем чтение логов агента...", "info")
...
def stop_agent():
    ...
    if agent_process:
        add_agent_log("🛑 Остановка агента по команде пользователя...", "warning")
        agent_process.terminate()
        try:
            agent_process.wait(timeout=5)
        except subprocess.TimeoutExpired:
            agent_process.kill()
            add_agent_log("⚠️ Агент был принудительно остановлен", "warning")
        agent_process = None
        add_agent_log("✅ Агент остановлен", "info")
...
def read_agent_logs():
    ...
    add_agent_log("📖 Запущен мониторинг логов агента...", "info")
        ...
        try:
            ...
            if line:
                ...
                if cleaned_line:
                    ...
                    add_agent_log(f"Агент: {cleaned_line}", log_type)
        except Exception as e:
            add_agent_log(f"Ошибка чтения логов агента: {e}", "error")
            break
    ...
    if agent_process:
        ...
        if return_code == 0:
            add_agent_log("✅ Агент завершил работу успешно", "success")
        elif return_code is None:
            add_agent_log("⚠️ Агент был остановлен", "warning")
        else:
            add_agent_log(f"❌ Агент завершил работу с кодом ошибки: {return_code}", "error")
