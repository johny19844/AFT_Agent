...
class TestAutomationAgent:
    ...
    def _is_file_changed(self, filename, current_sha, current_modified):
        ...
        tracked_info = self.file_tracking.get(filename)
        if not tracked_info:
            ...
            return True
        if tracked_info['sha'] != current_sha or tracked_info['last_modified'] != current_modified:
            self.file_tracking[filename] = {
                'sha': current_sha,
                'last_modified': current_modified
            }
        ...
        try:
            ...
            for file_path in all_txt_files:
                try:
                    ...
                    if self._is_file_changed(file_path, sha, last_modified):
                        changed_files.append(file_path)
                        logger.info(f"📝 File changed or new: {file_path}")
                except GithubException as e:
                    logger.error(f"❌ Error getting file info for {file_path}: {e}")
            logger.info(f"📊 Found {len(changed_files)} changed/new files: {changed_files}")
